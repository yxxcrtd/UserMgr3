 ...
 * </ul>
 *
 * 生成 ST(Service Ticket) 和  TGT(TicketGrantingTicket)
 */
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
    ...
    public void destroyTicketGrantingTicket(final String ticketGrantingTicketId) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("删除ticket:Removing ticket [" + ticketGrantingTicketId + "] from registry.");
        }
        ...
        log.debug("票证"+ticketGrantingTicketId+"设置为过期，就会通知各个客户端.....");
        ticket.expire();
    ...
    public String grantServiceTicket(final String ticketGrantingTicketId, final Service service, final Credentials credentials) throws TicketException {
        ...
        if (ticketGrantingTicket == null) {
        	/*票证得不到，没法知道是否过期，也就无法通知其他客户端产品注销*/
        	/*尝试再注册到this.ticketRegistry,会不会有错误？？？TODO TODO TODO WHY WHY WHY */
        	/*
        	if(credentials!=null){
        	...
        	*/
        	...
        	log.debug("ticketRegistry中没有找到票证TGT啊：ticketGrantingTicket==null");
            throw new InvalidTicketException();
        }

        synchronized (ticketGrantingTicket) {
            if (ticketGrantingTicket.isExpired()) {
            	log.debug("TGT过期了啊：ticketGrantingTicket isExpired");
                this.ticketRegistry.deleteTicket(ticketGrantingTicketId);
                throw new InvalidTicketException();
            }
        }
        ...
        if (credentials != null) {
        	log.debug("得到了credentials");
            try {
            	log.debug("尝试得到了authentication");
                ...
                if (!(authentication.getPrincipal().equals(originalAuthentication.getPrincipal()) && authentication.getAttributes().equals(originalAuthentication.getAttributes()))) {
                    log.debug("authentication.getPrincipal()="+authentication.getPrincipal().getId());
                    log.debug("originalAuthentication.getPrincipal()="+originalAuthentication.getPrincipal().getId());
        ...
        log.debug("grantServiceTicket:返回"+serviceTicket.getId());
    ...
    @Audit(
        ...
        @Profiled(tag = "GRANT_SERVICE_TICKET",logFailuresSeparately = false)
        @Transactional(readOnly = false)
        public String grantServiceTicketByCredentials(final String ticketGrantingTicketId,
            final Service service,final Credentials credentials) throws TicketException {
            return this.grantServiceTicket(ticketGrantingTicketId, service, credentials);
        }
