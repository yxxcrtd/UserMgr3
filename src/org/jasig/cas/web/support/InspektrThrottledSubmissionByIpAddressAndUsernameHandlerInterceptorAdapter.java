...
public class InspektrThrottledSubmissionByIpAddressAndUsernameHandlerInterceptorAdapter extends AbstractThrottledSubmissionHandlerInterceptorAdapter {
    ...
    protected boolean exceedsThreshold(final HttpServletRequest request) {
        ...
        log.debug(query);
        log.debug("getUsernameParameter()="+getUsernameParameter());
        final String userToUse = constructUsername(request, getUsernameParameter());
        log.debug("userToUse="+userToUse);
        final Calendar cutoff = Calendar.getInstance();
        cutoff.add(Calendar.SECOND, -1 * getFailureRangeInSeconds());
        if(this.jdbcTemplate!=null){
        	log.debug("this.jdbcTemplate.toString()="+this.jdbcTemplate.toString());
        	if(this.jdbcTemplate.getDataSource()!=null){
        		try {
					log.debug("this.jdbcTemplate.getDataSource.getConnection.toString()="+this.jdbcTemplate.getDataSource().getConnection().toString());
        ...
        if (failures.size() < 2) {
        	log.debug("exceedsThreshold=false");
            return false;
        }
